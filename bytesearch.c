...
static char HELP[]= {
   ...
   "\n"
   "Read contents of file/device <haystack> starting at offset <start>\n"
   "(defaults to 0) in chunks of fixed size <buffer_size> except for\n"
   "the last chunk which may be smaller. Search the contents of every\n"
   "chunk for a byte sequence <needle> read from standard input.\n"
   "\n"
   ...
   "measurements are bytes (not sectors, blocks, kB, MB etc).\n"
   "\n"
   "%s"
};
